...
def hls(image, src="RGB"):
    ...
    if is_list(image):
        ...
        for img in image_list:
            eval(
                "image_HLS.append(cv2.cvtColor(img,cv2.COLOR_" + src.upper() + "2HLS))"
            )
    else:
...
def bgr(image, src="RGB"):
    ...
    if is_list(image):
        ...
        for img in image_list:
            eval(
                "image_BGR.append(cv2.cvtColor(img,cv2.COLOR_" + src.upper() + "2BGR))"
            )
    else:
...
def rgb(image, src="BGR"):
    ...
    if is_list(image):
        ...
        for img in image_list:
            eval(
                "image_RGB.append(cv2.cvtColor(img,cv2.COLOR_" + src.upper() + "2RGB))"
            )
    else:
...
def green(image, src="BGR"):
    ...
    if is_list(image):
        ...
        image_green = eval(
            "cv2.cvtColor(image,cv2.COLOR_" + src.upper() + "2RGB)[:,:,1]"
        )
    return image_green
...
def blue(image, src="BGR"):
    ...
    if is_list(image):
        ...
        image_blue = eval(
            "cv2.cvtColor(image,cv2.COLOR_" + src.upper() + "2RGB)[:,:,2]"
        )
    return image_blue
...
def change_light(image, coeff):
    ...
    if coeff > 1:
        image_HLS[:, :, 1][
            image_HLS[:, :, 1] > 255
        ] = 255  ##Sets all values above 255 to 255
    else:
...
def brighten(image, brightness_coeff=-1):  ##function to brighten the image
    ...
    if is_list(image):
        ...
        for img in image_list:
            if brightness_coeff == -1:
                brightness_coeff_t = 1 + random.uniform(
                    0, 1
                )  ## coeff between 1.0 and 1.5
            else:
...
def random_brightness(image):
    ...
    if is_list(image):
        ...
        image_list = image
        for img in image_list:
            random_brightness_coefficient = 2 * np.random.uniform(
                0, 1
...
def add_shadow(
    image, no_of_shadows=1, rectangular_roi=(-1, -1, -1, -1), shadow_dimension=5
    ...
    if not (
        is_numeric(shadow_dimension)
        and shadow_dimension >= 3
        and shadow_dimension <= 10
    ):
        raise Exception(err_shadow_dimension)
    if (
        is_tuple(rectangular_roi)
        and is_numeric_list_or_tuple(rectangular_roi)
        and len(rectangular_roi) == 4
    ):
        x1 = rectangular_roi[0]
        y1 = rectangular_roi[1]
        x2 = rectangular_roi[2]
    ...
    if is_list(image):
        ...
        image_list = image
        for img in image_list:
            output = shadow_process(
                img, no_of_shadows, x1, y1, x2, y2, shadow_dimension
            )
...
def add_rain(
    ...
    rain_type="None",
):  ## (200,200,200) a shade of gray
    verify_image(image)
    slant_extreme = slant
    if not (
        is_numeric(slant_extreme)
        and (slant_extreme >= -20 and slant_extreme <= 20)
        or slant_extreme == -1
    ):
    ...
    if is_list(image):
        ...
        for img in image_list:
            output = rain_process(
                img, slant_extreme, drop_length, drop_color, drop_width, rain_drops
            )
            image_RGB.append(output)
    else:
        imshape = image.shape
        ...
        output = rain_process(
            image, slant_extreme, drop_length, drop_color, drop_width, rain_drops
        )
        image_RGB = output
...
def gravel_process(image, x1, x2, y1, y2, no_of_patches):
    ...
    for i in range(no_of_patches):
        ...
        rectangular_roi_default.append(
            (xx2, yy2, min(xx1, xx2 + 200), min(yy1, yy2 + 30))
        )
    img_hls = hls(image)
    for roi in rectangular_roi_default:
        ...
        for gravel in gravels:
            ...
            img_hls[
                max(y - r, 0) : min(y + r, y), max(x - r, 0) : min(x + r, x), 1
            ] = r1
...
def add_sun_process(
    ...
    for i in range(no_of_flare_circles):
        ...
        cv2.addWeighted(overlay, alpha, output, 1 - alpha, 0, output)
    output = flare_source(
        output, (int(flare_center[0]), int(flare_center[1])), src_radius, src_color
    )
    return output
...
def add_manhole(
    image, center=-1, color=(120, 120, 120), height=1, width=1, type="closed"
):  ##function to flip the image on horizontal axis
    verify_image(image)

    if center != -1:
        if not (
            is_tuple(center) and is_numeric_list_or_tuple(center) and len(center) == 2
        ):
